...
function TodoListComic() {
  ...
  const removeTodosComic = () => {
    const sure = window.confirm('本当にマンガのTodoを全て削除していいですか？');
    if (sure) {
      axios.delete('/api/v1/todos/destroy_comic')
      .then(response => {
        setTodos([])
      })
      .catch(e => {
